...
check_so = as.data.frame(results_matrix_so) %>% filter(x_a_init >= 44 & x_a_init <= 45) %>% filter(x_b_init >= 0 & x_b_init <= 1)
...
num_rows <- length(x_a_grid) * length(x_b_grid) * timesteps 
num_cols <- 8  # Number of variables you are storing
results_matrix_soa <- matrix(NA_real_, nrow = num_rows, ncol = num_cols)
colnames(results_matrix_soa) <- c("timestep", "x_a_init", "x_a", "e_a_star", "x_b_init", "x_b", "NPV_ea_star", "NPV_eb_star")
...
for(i in seq_along(x_a_grid)) {
  ...
  # Save results
  results_matrix_soa[row_counter, ] <- c(t, x_a_init, x_a, e_a_star, x_b_init, x_b, NPV_ea_star, NPV_eb_star)
  row_counter <- row_counter + 1
...
results_nar <-  results %>% 
  filter(accept > -0.5 & accept < 0.5)

#write.csv(results, here("data","outputs","D_ba_01_low_low.csv"), row.names=FALSE)
...
#results_int = results %>% rename(x_a = x_a_init) %>% rename(x_b = x_b_init) |> mutate(xab = x_a * x_b) 

#results = results %>% rename(x_a = x_a_init) %>% rename(x_b = x_b_init)

#interp_model <- mgcv::gam(accept ~ s(x_a) + s(x_b) + s(xab), data = results_int)

#interp_frame <-

#    data.frame(x_a = x_a, x_b = x_b, xab = x_a * x_b)

#accept <- pmax(min(results$accept),predict(interp_model, newdata = interp_frame))

# 1. Create a new dataframe for the grid values
#grid_df <- expand.grid(x_a = seq(0,100, by=0.1), x_b = seq(0,100, by=0.1))

# 2. Calculate the interaction term xab for the grid
#grid_df$xab <- grid_df$x_a * grid_df$x_b

# 3. Predict e_a_star for the entire grid
#predicted_accept <- predict(interp_model, newdata = grid_df)

# 4. Merge the interpolated e_a_star values with df_results_so_ea based on x_a and x_b
#df_results_interpolated <- merge(grid_df, results, by.x = c("x_a", "x_b"), by.y = c("x_a", "x_b"), all.x = TRUE)

# Replace the NAs in df_results_so_ea_interpolated with the interpolated values
#df_results_interpolated$accept <- ifelse(is.na(df_results_interpolated$accept), predicted_accept, df_results_interpolated$accept)

#df_results_int <- df_results_interpolated %>% select(x_a, x_b, accept) %>% 
#  filter(accept > -0.005 & accept < 0.005)

#print(df_results_int)
...
ggplot() +
  geom_point(data = results, aes(x = x_a_init, y = accept))
