...
# Calculate the number of iterations over M_ba
num_M_ba_iterations <- length(seq(0, 1, by = 0.1))

# Calculate the number of rows
num_rows <- length(x_a_grid) * length(x_b_grid) * timesteps * num_M_ba_iterations
num_cols <- 12
results_matrix_non <- matrix(NA_real_, nrow = num_rows, ncol = num_cols)
colnames(results_matrix_non) <- c("M_ba", "timestep", "x_a_init", "x_a", "h_a", "e_hat_a", "x_b_init", "x_b", "h_b", "e_hat_b", "NPV_a_hat", "NPV_b_hat")

row_counter <- 1

# Initialize variables to store NPV for each x_a and x_b combination
NPV_a_hat_total <- numeric(length = length(x_a_grid) * length(x_b_grid) * num_M_ba_iterations)
NPV_b_hat_total <- numeric(length = length(x_a_grid) * length(x_b_grid) * num_M_ba_iterations)
names(NPV_a_hat_total) <- paste0("x_a", 1:length(x_a_grid), "_x_b", 1:length(x_b_grid), "_M_ba", 1:num_M_ba_iterations)
names(NPV_b_hat_total) <- paste0("x_a", 1:length(x_a_grid), "_x_b", 1:length(x_b_grid), "_M_ba", 1:num_M_ba_iterations)
    ...
    for(i in seq_along(x_a_grid)) {
        for(j in seq_along(x_b_grid)) {
            ...
            for(t in 1:timesteps) {
                ...
                results_matrix_non[row_counter, ] <- c(M_ba, t, x_a_init, x_a, h_a_val, e_hat_values$e_hat_a, x_b_init, x_b, h_b_val, e_hat_values$e_hat_b, NPV_a_hat, NPV_b_hat)
            ...
            # Store NPV for the current x_a, x_b, and M_ba combination
            index <- (i - 1) * length(x_b_grid) * num_M_ba_iterations + (j - 1) * num_M_ba_iterations + round(M_ba * 10) + 1
...
colnames(results_matrix_so) <- c("M_ba", "timestep", "x_a_init", "x_a", "h_a", "e_star_a", "x_b_init", "x_b", "h_b","e_star_b", "NPV_a_star", "NPV_b_star")

row_counter <- 1

# Initialize variables to store NPV for each x_a and x_b combination
NPV_a_star_total <- numeric(length = length(x_a_grid) * length(x_b_grid)) * num_M_ba_iterations
NPV_b_star_total <- numeric(length = length(x_a_grid) * length(x_b_grid)) * num_M_ba_iterations
names(NPV_a_star_total) <- paste0("x_a", 1:length(x_a_grid), "_x_b", 1:length(x_b_grid), "_M_ba", 1:num_M_ba_iterations)
names(NPV_b_star_total) <- paste0("x_a", 1:length(x_a_grid), "_x_b", 1:length(x_b_grid), "_M_ba", 1:num_M_ba_iterations)
...
for(i in seq_along(x_a_grid)) {
  ...
  # Save results including e_hat_a and e_hat_b
  results_matrix_so[row_counter, ] <- c(M_ba, t, x_a_init, x_a, h_a_val, e_star_values$e_star_a, x_b_init, x_b, h_b_val, e_star_values$e_star_b, NPV_a_star, NPV_b_star)
  row_counter <- row_counter + 1
...
max_rows <- num_combinations * num_time_steps * num_M_ba_iterations

# Initialize NPV variables and counter
num_cols <- 12
results_matrix_soa <- matrix(NA_real_, nrow = max_rows, ncol = num_cols)
colnames(results_matrix_soa) <- c("M_ba", "timestep", "x_a_init", "x_a", "h_a_val", "e_a_star", "x_b_init", "x_b", "h_b_val", "V_a_star", "V_b_star", "NPV_ea_star")

row_counter <- 1

# Initialize NPV variables and counter
NPV_ea_star_total <- numeric(length = num_combinations * num_M_ba_iterations)
names(NPV_ea_star_total) <- paste0("x_a", 1:length(x_a_grid), "_x_b", 1:length(x_b_grid), "_M_ba", 1:num_M_ba_iterations)
  ...
  for(j in seq_along(x_b_grid)) {
    ...
    for(M_ba in seq(0, 1, by = 0.1)) {
      ...
      for(t in 0:timesteps) {
        ...
        # Save results
        results_matrix_soa[row_counter, ] <- c(M_ba, t, x_a_init, x_a, h_a_val, e_a_star, x_b_init, x_b, h_b_val, V_a_star, V_b_star, NPV_ea_star)
        row_counter <- row_counter + 1
      }
...
results <- df_results_non_bind %>% left_join(df_results_so_bind, by= c("M_ba", "x_a_init", "x_b_init")) %>% 
  left_join(df_results_ea_bind, by= c("M_ba","x_a_init", "x_b_init")) %>% 
...
write.csv(results, here("data","outputs","low_low","theta_Mba_rev.csv"), row.names=FALSE)
