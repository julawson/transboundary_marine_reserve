...
num_rows <- length(x_a_grid) * length(x_b_grid) * timesteps * length(seq(1, 1, by = 0.1)) * length(seq(0, 1, by = 0.1)) * length(seq(0, 1, by = 0.1)) * length(seq(0, 1, by = 0.1))
num_cols <- 11  
results_matrix_non <- matrix(NA_real_, nrow = num_rows, ncol = num_cols)
colnames(results_matrix_non) <- c("D_ab", "D_ba", "timestep", "x_a_init", "x_a", "e_hat_a", "x_b_init", "x_b", "e_hat_b", "NPV_a_hat", "NPV_b_hat")
...
for(i in seq_along(x_a_grid)) {
  ...
  results_matrix_non[row_counter, ] <- c(D_ab, D_ba, t, x_a_init, x_a, e_hat_values$e_hat_a, x_b_init, x_b, e_hat_values$e_hat_b, NPV_a_hat, NPV_b_hat)
...
colnames(results_matrix_so) <- c("D_ab", "D_ba", "timestep", "x_a_init", "x_a", "e_star_a", "x_b_init", "x_b", "e_star_b", "NPV_a_star", "NPV_b_star")
...
for(i in seq_along(x_a_grid)) {
  ...
  # Save results including e_hat_a and e_hat_b
  results_matrix_so[row_counter, ] <- c(D_ab, D_ba, t, x_a_init, x_a, e_star_values$e_star_a, x_b_init, x_b, e_star_values$e_star_b, NPV_a_star, NPV_b_star)
  row_counter <- row_counter + 1
...
# Calculate the number of iterations over D_ba and D_ab
...
max_rows <- num_combinations * num_time_steps * num_D_ba_iterations * num_D_ab_iterations

# Initialize NPV variables and counter
num_cols <- 13
results_matrix_soa <- matrix(NA_real_, nrow = max_rows, ncol = num_cols)
colnames(results_matrix_soa) <- c("D_ba", "D_ab", "timestep", "x_a_init", "x_a", "h_a_val", "e_a_star", "x_b_init", "x_b", "h_b_val", "V_a_star", "V_b_star", "NPV_ea_star")

row_counter <- 1

# Initialize NPV variables and counter
NPV_ea_star_total <- numeric(length = num_combinations * num_D_ba_iterations * num_D_ab_iterations)
names(NPV_ea_star_total) <- paste0("x_a", 1:length(x_a_grid), "_x_b", 1:length(x_b_grid), "_D_ba", 1:num_D_ba_iterations, "_D_ab", 1:num_D_ab_iterations)
  ...
  for(j in seq_along(x_b_grid)) {
    ...
    # Recalculate df_inf for every new D_ba value
      ...
      for(t in 0:timesteps) {
        ...
        # Save results
        results_matrix_soa[row_counter, ] <- c(D_ba, D_ab, t, x_a_init, x_a, h_a_val, e_a_star, x_b_init, x_b, h_b_val, V_a_star, V_b_star, NPV_ea_star)
        row_counter <- row_counter + 1
      }
    
    # Store NPV for the current x_a and x_b combination
        index <- (i - 1) * length(x_b_grid) * num_D_ba_iterations * num_D_ab_iterations + 
                 (j - 1) * num_D_ba_iterations * num_D_ab_iterations + 
                 round(D_ba * 10) + 1 + 
                 round(D_ab * 10) * num_D_ba_iterations + 1
...
results <- df_results_non_bind %>% left_join(df_results_so_bind, by= c("D_ab", "D_ba", "x_a_init", "x_b_init")) %>% 
  left_join(df_results_ea_bind, by= c("D_ab", "D_ba","x_a_init", "x_b_init")) %>% 
...
write.csv(results, here("data","outputs","mvmt_dispersal.csv"), row.names=FALSE)
