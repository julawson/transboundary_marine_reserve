...
num_rows <- length(x_a_grid) * length(x_b_grid) * timesteps * length(seq(0, 1, by = 0.1)) * length(seq(0, 1, by = 0.1))
num_cols <- 11  
results_matrix_non <- matrix(NA_real_, nrow = num_rows, ncol = num_cols)
colnames(results_matrix_non) <- c("M_ab", "M_ba", "timestep", "x_a_init", "x_a", "e_hat_a", "x_b_init", "x_b", "e_hat_b", "NPV_a_hat", "NPV_b_hat")
...
for(i in seq_along(x_a_grid)) {
  ...
  results_matrix_non[row_counter, ] <- c(M_ab, M_ba, t, x_a_init, x_a, e_hat_values$e_hat_a, x_b_init, x_b, e_hat_values$e_hat_b, NPV_a_hat, NPV_b_hat)
...
colnames(results_matrix_so) <- c("M_ab", "M_ba", "timestep", "x_a_init", "x_a", "e_star_a", "x_b_init", "x_b", "e_star_b", "NPV_a_star", "NPV_b_star")
...
for(i in seq_along(x_a_grid)) {
  ...
  # Save results including e_hat_a and e_hat_b
  results_matrix_so[row_counter, ] <- c(M_ab, M_ba, t, x_a_init, x_a, e_star_values$e_star_a, x_b_init, x_b, e_star_values$e_star_b, NPV_a_star, NPV_b_star)
  row_counter <- row_counter + 1
...
max_rows <- num_combinations * num_time_steps * num_M_ba_iterations * num_M_ab_iterations

# Initialize NPV variables and counter
num_cols <- 11
results_matrix_soa <- matrix(NA_real_, nrow = max_rows, ncol = num_cols)
colnames(results_matrix_soa) <- c("M_ba", "M_ab", "timestep", "x_a_init", "x_a", "h_a_val", "e_a_star", "x_b_init", "x_b", "h_b_val", "NPV_ea_star")
  ...
  for(j in seq_along(x_b_grid)) {
    ...
    # Recalculate df_inf for every new M_ba value
      ...
      for(t in 1:timesteps) {
        ...
        # Save results
        results_matrix_soa[row_counter, ] <- c(M_ba, M_ab, t, x_a_init, x_a, h_a_val, e_a_star, x_b_init, x_b, h_b_val, NPV_ea_star)
        row_counter <- row_counter + 1
      }
    
    # Store NPV for the current x_a and x_b combination
        index <- (i - 1) * length(x_b_grid) * num_M_ba_iterations * num_M_ab_iterations + 
                 (j - 1) * num_M_ba_iterations * num_M_ab_iterations + 
                 round(M_ba * 10) + 1 + 
                 round(M_ab * 10) * num_M_ba_iterations + 1
...
write.csv(results, here("data","outputs","mvmt_dispersal_adult_rev2.csv"), row.names=FALSE)
