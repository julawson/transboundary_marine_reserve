...
colnames(results_matrix_soa) <- c("timestep", "x_a_init", "x_a", "e_a_star", "h_a_val", "x_b_init", "x_b", "h_b_val", "V_a_star","NPV_ea_star")
...
matrix_df_V = dcast(df_inf, x_a ~ x_b, value.var = "V_star")
rownames(matrix_df_V) = x_a_grid
matrix_df_V = matrix_df_V[, -1]
  ...
  for(j in seq_along(x_b_grid)) {
    ...
    for(t in 0:timesteps) {
      if (t == 0) {
        ...
        V_a_star <- compute_v_a_star(x_a = x_a_new, x_b = x_b_new, df_inf)
      ...
      # Save results
      results_matrix_soa[row_counter, ] <- c(t, x_a_init, x_a, e_a_star, h_a_val, x_b_init, x_b, h_b_val, V_a_star, NPV_ea_star)
      row_counter <- row_counter + 1
    }
...
results_compare <- results %>% select(x_a_init, x_a_non, h_a_non, e_hat_a, x_a_so, h_a_so, e_star_a, x_a_soa, h_a_soa, e_a_star, NPV_a_hat, NPV_a_star, NPV_ea_star, WTP_A, diff_coop) %>% distinct()
