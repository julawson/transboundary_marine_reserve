...
colnames(results_matrix_soa) <- c("M_ab", "M_ba", "D_ab", "D_ba", "timestep", "x_a_init", "x_a", "e_a_star", "x_b_init", "x_b", "NPV_ea_star", "NPV_eb_star")
...
for(i in seq_along(x_a_grid)) {
  ...
  # Save results
  results_matrix_soa[row_counter, ] <- c(M_ab, M_ba, D_ab, D_ba, t, x_a_init, x_a, e_a_star, x_b_init, x_b, NPV_ea_star, NPV_eb_star)
  row_counter <- row_counter + 1
...
results <- df_results_non_bind %>% left_join(df_results_so_bind, by= c("M_ab", "M_ba", "D_ab", "D_ba", "x_a_init", "x_b_init")) %>% 
  left_join(df_results_ea_bind, by= c("M_ab", "M_ba", "D_ab", "D_ba","x_a_init", "x_b_init")) %>% 
...
write.csv(results, here("data","outputs","mvmt_pt10.csv"), row.names=FALSE)
