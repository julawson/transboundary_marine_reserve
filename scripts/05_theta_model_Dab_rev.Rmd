...
colnames(results_matrix_soa) <- c("D_ab", "timestep", "x_a_init", "x_a", "h_a_val", "e_a_star", "x_b_init", "x_b", "h_b_val", "NPV_ea_star")
  ...
  for(j in seq_along(x_b_grid)) {
    ...
    for(D_ab in seq(0, 1, by = 0.1)) {
      ...
      for(t in 1:timesteps) {
        ...
        # Save results
        results_matrix_soa[row_counter, ] <- c(D_ab, t, x_a_init, x_a, h_a_val, e_a_star, x_b_init, x_b, h_b_val, NPV_ea_star)
        row_counter <- row_counter + 1
      }
...
write.csv(results, here("data","outputs","high_high","theta_Dab_rev2.csv"), row.names=FALSE)
