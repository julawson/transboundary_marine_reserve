...
M_ab_sens = read.csv(here("data","outputs","low_low","theta_Mab_rev.csv")) %>% add_column(theta = "M_ab") %>% rename(value = M_ab)
M_ba_sens = read.csv(here("data","outputs","low_low","theta_Mba_rev.csv")) %>% add_column(theta = "M_ba") %>% rename(value = M_ba)
D_ab_sens = read.csv(here("data","outputs","low_low","theta_Dab_rev.csv")) %>% add_column(theta = "D_ab") %>% rename(value = D_ab)
D_ba_sens = read.csv(here("data","outputs","low_low","theta_Dba_rev.csv")) %>% add_column(theta = "D_ba") %>% rename(value = D_ba)
...
low_low_orig <- low_low_df %>% filter(theta == "D_ab" & theta == "D_ba" & theta == "M_ba" & theta == "M_ab" | value==0.2)
low_high_orig <- low_high_df %>% filter(theta == "D_ab" & theta == "D_ba" | value==0.8 & theta == "M_ab" & theta == "M_ba" | value==0.2) 
high_low_orig <- high_low_df %>% filter(theta == "D_ab" & theta == "D_ba" | value==0.2 & theta == "M_ab" & theta == "M_ba" | value==0.8) 
high_high_orig <- high_high_df %>% filter(theta == "D_ab" & theta == "D_ba" & theta == "M_ab" & theta == "M_ba" | value==0.8)
...
ggsave(here("visuals","output","bar_plot_original_rev.jpg"), bar_plot_original, width = 8, height = 6)
...
low_low_df$theta <- factor(low_low_df$theta, levels = label_order)
...
geom_start = geom_ribbon %>% 
  filter(theta == "M_ba" & value == 0.2 | theta == "M_ab" & value == 0.2 | theta == "D_ba" & value == 0.2 |theta == "D_ab" & value == 0.2)
...
low_low <- ggplot(geom_ribbon, aes(x = value, y = mean_accept, color = theta)) +
  ...
  labs(x = "Theta Value", y = "Agreement Value") +
  scale_color_manual(
    name = "Theta",
    labels = c("Movement (A to B)", "Movement (B to A)", "Dispersal (A to B)", "Dispersal (B to A)"),
...
ggsave(here("visuals","output","params_summary_ordered_rev.jpg"), combined_plot, width = 15, height = 9)
...
theta_stacked3 <- mvmt_sens %>% 
  ...
  filter(scenario != "low_low" | theta!="M_ab" | value!=0.9) %>% 
  filter(scenario != "low_low" | theta!="M_ab" | value!=1)

theta_stacked_ll <- mvmt_sens %>% 
  mutate(NPV_non = NPV_a_hat + NPV_b_hat) %>% 
  select(theta, value, scenario, NPV_non, NPV_coop, NPV_ea_star) %>% 
  mutate(NPV_res_to_coop = ((NPV_ea_star - NPV_coop)/NPV_coop)*100) %>% 
  select(theta, value, scenario, NPV_res_to_coop) %>% 
  ...
  group_by(theta, value, scenario, Category) %>% 
  summarize(mean_Value_X = mean(Value_X)) %>% 
  filter(scenario == "low_low") %>% filter(theta=="M_ab") %>% filter(value>=0.9)

pal2 <- c("#7b3294", "#c2a5cf", "#fdb863", "#e66101")

low_low_check <- low_low_df %>% filter(theta == "M_ab") %>% group_by(theta, value) %>% summarize(mean_accept = mean(accept)) 

diff_npv <- ggplot() +
  geom_line(data = theta_stacked_ll, aes(x = value, y = mean_Value_X, color = scenario, 
                                       linetype = case_when(
                                         scenario == "low_low" & theta == "M_ab" & value >= 0.9 ~ "1",
                                         TRUE ~ "2")), size=1.5) +
  geom_line(data = theta_stacked3, aes(x = value, y = mean_Value_X, color = scenario, 
                                       linetype = case_when(
                                         scenario == "high_high" & theta == "M_ba" & value <= 0.1  ~ "1",
                                         ...
                                         scenario == "low_low" & theta == "D_ab" & value <= 0.4  ~ "1",
                                         scenario == "low_low" & theta == "D_ba" & value <= 0.3  ~ "1",
                                         TRUE ~ "2")), size=1.5) +
  geom_line(data = theta_stacked3, aes(x = value, y = mean_Value_X, color = scenario, 
                                       linetype = case_when(
                                         scenario == "high_high" & theta == "M_ba" & value >= 0.1  ~ "2",
                                         ...
                                         scenario == "low_low" & theta == "D_ab" & value >= 0.4  ~ "2",
  ...
  labs(x = "Theta Value", y = "Change in Net Present Value (%)") +
  theme_minimal() +
...
diff_stock <- ggplot() +
  ...
  labs(x = "Theta Value", y = "Change in Total Stock (%)") +
  theme_minimal() +
  geom_hline(yintercept=0, linetype="dashed") +
  facet_wrap(~theta, scales = "free", ncol = 2, labeller = labeller(theta = c(
  "D_ab" = "Dispersal to Reserve Patch (A to B)",
...
ggsave(here("visuals","output","coop_to_res_summary_rev.jpg"), coop_to_res, width = 15, height = 5)
